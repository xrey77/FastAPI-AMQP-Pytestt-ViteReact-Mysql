...
@pytest.mark.asyncio
async def test_update_profile_success(mocker, mock_db_session, mock_auth):
    ...
    mocker.patch("app.api.endpoints.updateprofile.user_update_profile", return_value=mock_user)

    mock_exchange = AsyncMock()
    app.state.rmq_exchange = mock_exchange

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        profile_data = {"firstname": "Batman", "lastname": "securepassword", "mobile": "23423423"}
        response = await ac.patch("/api/updateprofile/1/", json=profile_data, headers={"Authorization": "Bearer {token}"})

    assert response.status_code == 200
    assert response.json() == mock_user_data
    
    mock_exchange.publish.assert_called_once()
    assert mock_exchange.publish.call_args.kwargs["routing_key"] == "auth.updateprofile.success"
